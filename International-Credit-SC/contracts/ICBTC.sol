...
/**
 * @title ICBTC
 * @dev International Credit Bitcoin Backed Token
 * @notice ERC20 token representing Bitcoin reserves in the IC system
 */
contract ICBTC is ERC20, ERC20Burnable, AccessControl {
    ...
    function setICController(address _icController) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(_icController != address(0), "ICBTC: ICController cannot be zero address");
    ...
    function _update(address from, address to, uint256 value) internal override {
        ...
        if (from != address(0) && to != address(0) && address(icController) != address(0)) {
            require(icController.isKycPassed(from), "ICBTC: Sender not KYC approved");
            require(icController.isKycPassed(to), "ICBTC: Recipient not KYC approved");
        }
